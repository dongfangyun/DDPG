...
class CarEnv:
    ...
    def reset(self):
        ...
        self.location_player = self.location_start # 初始位置的carla.location
        self.dist_to_end_init = self.location_player.distance(self.destination) # 初始位置距目标的carla.distance

        self.world.tick() # 上一处tick确保车落下，此处往初始状态里传入

    def shotgundagger1(self):
        # 矫正专家，如果最近路点与局部路线规划下一个路点lane_id不符，重新规划路线, 目标点不变（不重新取点）。 排除交叉路口
    ...
    def caculate_reward(self, dist_to_start, dist_to_start_old, dist_to_end, dist_to_end_old, velocity, done, inva_lane):
        reward = 0.0
        reward += np.clip(dist_to_start - dist_to_start_old, -10.0, 10.0) # 此处当用曼哈顿距离
        reward += np.clip(dist_to_end_old - dist_to_end, -10.0, 10.0) 
    ...
    def step(self, action, episode_steps, dist_to_start_old, dist_to_end_old): 
        ...
        location_point = (self.location_player.x,self.location_player.y, self.location_player.z)

        forward_vector = (forward_vector_player.x, forward_vector_player.y, forward_vector_player.z)

        # 车辆初始点/终点位置及距初始点/终点距离
        start_point = (self.location_start.x, self.location_start.y, self.location_start.z)  
        destination_point = (self.destination.x, self.destination.y , self.destination.z)
        ...
        # # 设置智能专家，会修正导航 Shothundagger1
        # shotgundagger1()

        # # 设置偏航重置 Shothundagger2
        # shotgundagger2()

        # 根据当前这一步执行专家指示
        self.act_expert = self.agent.run_step() 
        ...
        # action.shape([1,2]):  油门刹车action[0][0]:(-1,1) 方向盘action[0][1]:(-1,1)
        ...
        reward = self.caculate_reward(dist_to_start, dist_to_start_old, dist_to_end, dist_to_end_old, velocity, self.done, self.invation)

        # len(data) = 19 + reward = 20
        data = [*location_point, *start_point, *destination_point, *forward_vector, velocity, *acceleration, *angular_velocity, reward]

        return  self.done, data, self.act_expert, dist_to_start, dist_to_end
